...
interface HeaderProps {
  ...
  mobileMenuContent?: (controls: { closeMenu: () => void }) => ReactNode
}

const githubHref = 'https://github.com/FourWindff'
...
export default function Header({
  ...
  const shellClassName = isHome ? 'theme-header-shell-home' : 'theme-header-shell theme-border border-b'
  const reserveSpace = !isHome
  const mobileMenuButtonClassName = isHome
    ? 'theme-text-primary md:hidden mr-3 p-4 dark:mix-blend-difference'
    : 'theme-text-primary md:hidden mr-3 p-2'
  const brandClassName = isHome
    ? 'theme-text-primary flex items-center gap-3 text-xl font-black uppercase tracking-tighter dark:mix-blend-difference'
    : 'theme-text-primary flex items-center gap-3 text-xl font-black uppercase tracking-tighter hover:opacity-70 transition-opacity'
  const desktopNavClassName = isHome ? 'ml-auto hidden md:flex items-center gap-12' : 'ml-auto hidden md:flex items-center gap-8'
  const desktopItemClassName = isHome
    ? 'theme-text-primary text-sm uppercase tracking-[0.2em] font-bold dark:mix-blend-difference hover:opacity-70 transition-opacity'
    : 'theme-text-primary text-sm uppercase tracking-[0.2em] font-bold hover:opacity-70 transition-opacity'
  const themeButtonClassName = isHome
    ? 'theme-text-primary p-2 dark:mix-blend-difference hover:opacity-70 transition-opacity'
    : 'theme-text-primary p-2 hover:opacity-70 transition-opacity'
  const mobilePanelHeaderClassName = isHome
    ? 'theme-border flex items-center gap-3 border-b px-4 sm:px-6 py-6'
    : 'theme-border flex items-center gap-3 border-b px-4 sm:px-6 py-4'
    ...
    <>
      ...
      >
        <nav className="relative">
          <div className="flex items-center px-4 sm:px-6 xl:px-12 2xl:px-16 py-4">
            ...
            <div className={desktopNavClassName}>
              {isHome ? (
                <>
                  <Link to="/archive" className={desktopItemClassName}>
                    Archive
                  </Link>
                  <a
                    href={githubHref}
                    ...
                    className={`${desktopItemClassName} flex items-center gap-2`}
                  >
                    <GitHubMark size={16} />
                    Github
                  </a>
                ...
                <Link to={resolvedBackTo} className={`${desktopItemClassName} flex items-center gap-2`}>
                  <ArrowLeft size={16} />
                  {resolvedBackLabel}
                </Link>
              )}
