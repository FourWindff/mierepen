...
const NavItem = ({ label, href }: { label: string; href: string }) => (
  <a
    href={href}
    className="text-sm uppercase tracking-[0.2em] font-bold text-white mix-blend-difference hover:opacity-70 transition-opacity"
  >
    {label}
  </a>
);
    ...
    <div className="min-h-screen bg-[#080808] text-white font-sans selection:bg-white selection:text-black">
      ...
      <main>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
            ...
            <div className="lg:col-span-8">
              <h2 className="text-4xl font-bold mb-8 leading-tight">
                I'm a digital architect bridging the gap between <span className="text-neutral-400 italic">low-fidelity aesthetics</span> and <span className="text-neutral-400 italic">high-performance systems.</span>
              </h2>
        ...
        <section className="max-w-7xl mx-auto px-12 py-24 border-t border-x border-white/10 bg-white/[0.01]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-1 px-1 bg-white/10 border border-white/10">
            {[
        ...
        <section className="max-w-7xl mx-auto px-12 py-24 border-t border-x border-white/10">
          ...
          <div className="space-y-4">
            {[
              ...
              <a key={i} href="#" className="flex items-center justify-between p-6 border border-white/5 hover:border-white/40 transition-colors group">
                <div className="flex items-center gap-8">
                  <span className="font-mono text-[10px] text-white/20">#{122 - i}</span>
                  <span className="text-lg font-bold group-hover:translate-x-2 transition-transform">{title}</span>
                </div>
                <span className="font-mono text-[10px] text-white/20">01.2024</span>
              </a>
            ))}
          </div>
        </section>
        ...
        <section className="max-w-7xl mx-auto px-12 py-24 border-t border-x border-white/10 grid grid-cols-1 lg:grid-cols-2 gap-12">
          <aside className="space-y-12">
            <section>
              <h4 className="font-mono text-[10px] uppercase tracking-widest text-white/50 mb-6 font-bold">
                Search_Registry
              </h4>
              <div className="relative border-b border-white/20 focus-within:border-white transition-colors">
                ...
                />
                <Search size={16} className="absolute right-4 top-1/2 -translate-y-1/2 opacity-30" />
              </div>
            </section>
          </aside>

          <section className="bg-white/5 p-12">
            <h4 className="font-mono text-[10px] uppercase tracking-widest text-white/50 mb-6 font-bold">
              Newsletter_Sub
            </h4>
            ...
            <div className="space-y-4">
              <input
                ...
                className="w-full bg-transparent border-b border-white/20 py-4 font-mono text-sm focus:outline-none focus:border-white transition-colors"
              />
              <button className="w-full bg-white text-black font-black uppercase text-xs py-4 hover:bg-neutral-200 transition-colors tracking-[0.2em]">
                Initialize_Sub
              </button>
            </div>
          </section>
      ...
      <footer className="px-12 py-10 border-t border-white/10">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-4">
            <span className="font-mono text-[10px] uppercase tracking-widest text-white/50">System Status: Optimal</span>
          </div>

          <div className="flex gap-8">
            <a href="#" className="p-2 text-white/30 hover:text-white transition-colors"><Code size={20} /></a>
            <a href="#" className="p-2 text-white/30 hover:text-white transition-colors"><AtSign size={20} /></a>
            <a href="#" className="p-2 text-white/30 hover:text-white transition-colors"><Mail size={20} /></a>
          </div>
